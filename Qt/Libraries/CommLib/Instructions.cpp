...
{
	QList<quint8> result;
	result << ANNOUNCEMENT << SEND_STRATEGIES << CONFIGURATION_FILES << CONFIGURATION_FILE_DATA
		   << SERIAL_PORTS << AX12_POSITIONS << STRATEGY_STATUS << AUTO_STRATEGY_INFO << AX12_MVT_FILE << CONFIGURATION_FILE_EVENT;
	return result;
}
...
QList<quint8> Instruction::clientToServerInstructions()
{
	...
	result << CONNECT << DISCONNECT << UPDATE_SERVER << PING_SERVER
		   << ASK_STRATEGIES << ASK_FILES << ASK_FILE_DATA << SET_FILE_DATA
